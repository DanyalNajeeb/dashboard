...
export class CreatedComponent implements OnInit {
  ...
  constructor(
    ...
    ELEMENT_DATA = this.orderService.getNewOrder();
  ...
  initDrillDown(element: any) {
    ...
    this.onLoadLatitude = element["pickup"].location.latitude;
    this.onLoadLongitude = element["pickup"].location.longitude;
  ...
  commentPopUp(order: any) {
    ...
    const dialogRef = this.dialog.open(CommentPanelComponent, {
      width: "300px",
      data: { _id: this.orderNo, timeStamp: this.timeStamp, status: "created" },
    });
  ...
  updateOrder(order) {
    ...
    const dialogRef = this.dialog.open(UpdateComponent, {
      width: "70%",
      data: { _id: this.orderNo, timeStamp: this.timeStamp,data:this.order }
    });
    dialogRef.afterClosed().subscribe((dialogResult) => {
      this.result = dialogResult;
    });
